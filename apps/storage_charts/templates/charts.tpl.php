...
OCP\Util::addScript('storage_charts', 'highCharts/highcharts');
OCP\Util::addScript('storage_charts', 'chartsUsedStorage');
...
<div id="storage-charts">
	<table class="topblock"><tr>
		<td class="company">DjazzLab Storage Charts<?php echo'<br> &nbsp; v'.OC_Appconfig::getValue('storage_charts','installed_version').' / '.OC_Util::getVersionString()?></td>
		<td class="message"><?php print $l->t('Drag\'N\'Drop on the chart title to re-order'); ?></td>
		<td class="settings">
			<a title="<?php echo $l->t('Settings');?>">
			<img class="svg" src="/oc/core/img/actions/settings.svg" alt="<?php echo $l->t('Settings');?>"/>
			</a>
		</td></tr>
	</table>
	...
	<div id="stc_sortable">
		<?php foreach($_['sc_sort'] as $sc_sort){
			...
			<div id="<?php print($sc_sort); ?>" class="personalblock">
				<h3><img src="<?php print(OCP\Util::imagePath('storage_charts', 'move.png')); ?>" /><?php print($l->t($sc_sort_title).' '.$l->t('for')); ?> "<?php print(OC_Group::inGroup(OCP\User::getUser(), 'admin')?$l->t('all users'):OCP\User::getUser()); ?>"
