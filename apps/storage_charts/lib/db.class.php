...
//	                       ------------
		OCP\Util::writeLog('storage_charts', self::milliStamp().' begin ', OCP\Util::DEBUG);
...
//			Update or insert user's storage into Database (at most one record per day)
			$myQuery = OCP\DB::prepare('SELECT stc_id FROM *PREFIX*dlstcharts WHERE oc_uid=? AND stc_dayts=?');
			$myResult = $myQuery->execute(Array($myUserID, mktime(0,0,0)))->fetchRow();
			if ( $myResult ) {
				$myQuery = OCP\DB::prepare('UPDATE *PREFIX*dlstcharts SET stc_used=?, stc_total=? WHERE stc_id=?');
				$myQuery->execute(Array($myUsed, $myTotal, $myResult['stc_id']));
			} else {
				$myQuery = OCP\DB::prepare('INSERT INTO *PREFIX*dlstcharts (oc_uid,stc_month,stc_dayts,stc_used,stc_total) VALUES (?,?,?,?,?)');
				$myQuery->execute(Array($myUserID, date('Ym'), mktime(0,0,0), $myUsed, $myTotal));
			}
			OCP\Util::writeLog('storage_charts', self::milliStamp().' calc  '." $myUsed/$myTotal ", OCP\Util::DEBUG);
		}
		$_SESSION['must_dirstat'] = false;
		OCP\Util::writeLog('storage_charts', self::milliStamp().' end   ', OCP\Util::DEBUG);
	}
